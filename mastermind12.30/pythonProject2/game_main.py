...
class Game:
    ...
    def handle_click(self, x, y):
        ...
        if check_button_x[0] <= x <= check_button_x[1] and check_button_y[0] <= y <= check_button_y[1]:
            result, self.game_count = self.game_func.confirm_answer(self.need_check_answer, self.correct_color, self.game_count, self.m_list, self.small_m_list)
            ...
            self.game_func.delete_answer(self.game_count, self.m_list)
